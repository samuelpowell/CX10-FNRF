...
#include "config.h"
 ...
 // RF buffer
 static char rxbuffer[RF_PAYLOADSIZE];
...
void init_rf(){
    ...
    nrfWrite1Reg(REG_RF_CH, RF_BIND_CHANNEL);       // Set to RF BIND channel
 
    #ifdef RF_CHSET_XN297
    ...
    nrfWrite1Reg(REG_STATUS, NRF_STATUS_CLEAR);     // Clear status
    nrfWrite1Reg( REG_FIFO_STATUS, 0x00);           // Clear FIFO bits
    
    // We we need to activate feature before we do this, so presumably this does nothing?
    ...
    nrfWrite1Reg(REG_RX_PW_P0, RF_PAYLOADSIZE);     // Set payload size on all RX pipes
    nrfWrite1Reg(REG_RX_PW_P1, RF_PAYLOADSIZE);
    nrfWrite1Reg(REG_RX_PW_P2, RF_PAYLOADSIZE);
    nrfWrite1Reg(REG_RX_PW_P3, RF_PAYLOADSIZE);
    nrfWrite1Reg(REG_RX_PW_P4, RF_PAYLOADSIZE);
    nrfWrite1Reg(REG_RX_PW_P5, RF_PAYLOADSIZE);
    
    // Check for Beken BK2421/BK2423 chip using activate code 0x53.
    // This has no effect when executed on a genuine nrf24L01 nor on
    // a Panchip XN297.
    nrfActivateBK2423();
...
bool rx_rf(int16_t *RXcommands) 
{
    ...
    if(nrfGetStatus() & 0x40)
    {
        ...
        nrfReadRX(rxbuffer, RF_PAYLOADSIZE);
...
void bind_rf() {
    ...
    while(!bind) {
        ...
        while (!(nrfRead1Reg(REG_FIFO_STATUS) & 0x01))
        {
            ...
            if(nrfRxLength(0) == RF_PAYLOADSIZE) nrfReadRX(rxbuffer, RF_PAYLOADSIZE);
        }
        RADIO_DIS_CE();
  ...
  while(!bind) {
    ...
    while(!(nrfGetStatus() & 0x40));
      ...
      while (!(nrfRead1Reg(REG_FIFO_STATUS) & 0x01)) {
        ...
        // Bind packet is nine bytes on pipe zero
        if(nrfRxLength(0) == RF_PAYLOADSIZE) {
